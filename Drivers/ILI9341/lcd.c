...
{
#if  ENANLE_SOFTWARE_SPI==0
	HAL_SPI_Transmit(&HARDWARE_SPI_CHANNEL, &Data, sizeof(Data), HAL_MAX_DELAY);
#elif ENANLE_SOFTWARE_SPI==1//如果适用软件SPI
	unsigned char i=0;
	for(i=8;i>0;i--)
	{
	  if(Data&0x80)	
...
//*************3.2inch ILI9341初始化**********//
  ...
  LCD_direction(USE_HORIZONTAL);//设置LCD显示方向
	HAL_GPIO_WritePin(LCD_LED_GPIO_Port,LCD_LED_Pin,GPIO_PIN_SET);//点亮背光	,对其不做PWM调光处理，直接以最大输出
